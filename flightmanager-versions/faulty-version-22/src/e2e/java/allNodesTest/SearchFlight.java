	...
	public void test1() {
		...
		Warp.initiate(new Activity() {
			...
			public void perform() {
				...
				Date date = new Date();
				flightDetailsPage.searchAnotherFlight(pisaAirport.getCityName(), parigiAirport.getCityName(), date,
						"1");
				...
				assertEquals(pisaAirport.getCityName(), displayedSourceAirport);
				assertEquals(parigiAirport.getCityName(), displayedDestinationAirport);
			}
		}).inspect(new Inspection() {
			private static final long serialVersionUID = 1L;
		});

		// alternative way of assertion
//		Warp.initiate(new Activity() {
//			@Override
//			public void perform() {
...
//			
//			@Inject
//			FlightManagerComponent flightManagerComponent;
//			
//			@BeforeServlet
//			public void checkNumberOfresultingFlights() {
//				assertTrue(flightManagerComponent.existsResult());
//			}
//		});
		...
		Warp.initiate(new Activity() {
			...
			public void perform() {
				...
				float totalPriceWithoutDiscount = bookingDetailsPage.getTotalPriceWithoutDiscount();
				float pricePerPerson = pisaPalermoFlight.getPricePerPerson();
				float expectedFeeRatio = (float) 0.225;

				float expectedPriceNoFee = pricePerPerson * numOfPassengers;
				float expectedFee = expectedPriceNoFee * expectedFeeRatio;
				float expectedTotalPrice = expectedPriceNoFee + expectedFee;

				assertEquals(expectedTotalPrice, totalPriceWithoutDiscount, 0.2);
			}
		}).inspect(new Inspection() {
			private static final long serialVersionUID = 1L;
		});
