...
import it.unifi.ing.swam.controller.BookingViewController;
...
public class VisitorManageBooking {
	...
	public void setUp() {
		...
		// TODO: fix manual booking insertion and use that instead
		// search flight
		/*Warp.initiate(new Activity() {
			...
			public void perform() {
				...
				homePage.searchFlights(pisaAirport.getCityName(), palermoAirport.getCityName(), new Date(), "1");
			}
		}).inspect(new Inspection() {
		...
		Warp.initiate(new Activity() {
			...
			public void perform() {
				...
				FlightsResult flightResultsPage = new FlightsResult(driver);
				flightResultsPage.clickDefaultConfirmFlightButton();
			}
		}).inspect(new Inspection() {
		...
		Warp.initiate(new Activity() {
			...
			public void perform() {
				...
				BookingDetails bookingDetailsPage = new BookingDetails(driver);
				
				String fooPassengerBirthDate = new SimpleDateFormat("dd/MM/yyyy")
						.format(fooPassenger.getDateOfBirth());
				
				bookingDetailsPage.compileBookingDataFormAndConfirm(
						pisaPalermoBooking.getEmail(),
						fooPassenger.getFirstName(),
						fooPassenger.getSurname(),
						fooPassenger.getIdCard(),
						fooPassengerBirthDate);
		...
		Warp.initiate(new Activity() {
			...
			public void perform() {
				...
				homePage.searchFlights(palermoAirport.getCityName(), pisaAirport.getCityName(), new Date(), "1");
			}
		}).inspect(new Inspection() {
		...
		Warp.initiate(new Activity() {
			...
			public void perform() {
				...
				String barPassengerBirthDate = new SimpleDateFormat("dd/MM/yyyy")
						.format(barPassenger.getDateOfBirth());
				
				bookingDetailsPage.compileBookingDataFormAndConfirm(
						palermoPisaBooking.getEmail(),
						barPassenger.getFirstName(),
						barPassenger.getSurname(),
						barPassenger.getIdCard(),
						barPassengerBirthDate);
				bookingDetailsPage.clickBackButton();
			}
		}).inspect(new Inspection() {
	...
	public void test1() {
		...
		Warp.initiate(new Activity() {
			...
			@BeforeServlet
			public void checkSelectedBooking() {
				assertNull(bookingViewController.getSelectedBooking().getId());
			}
		});
