...
public class VisitorBookFlight {
	...
	public void setUp() {
		...
		/*
		 * User user = ModelFactory.user(); user.setUserRole(UserRole.Registered);
		 * user.setUsername(username);
		 * user.setPassword(PasswordManagerComponent.encode(userPwd));
		 * user.setHomeCountry(italy); Calendar calendar = Calendar.getInstance();
		...
		/*
		 * Booking booking; for (int i = 0; i < 9; i++) { booking = new Booking();
		 * booking.setRegisteredUserOwner(user); em.persist(booking); }
		 */
		...
		Warp.initiate(new Activity() {
			...
			public void perform() {
				...
				/*
				 * TODO: start at home, search, and confirm a flight REASON: a visitor user
				 * can't go directly to booking-details
				 */
				/*
				...
				/*
				 * int flights = flightResultsPage.getTotalFlightVisualized(); assertEquals(1,
				 * flights);
				 */
			}
	...
	public void initializeBeansState() {
		...
		Warp.initiate(new Activity() {
			...
			@Inject
			VisitorBookingController bookingController;
