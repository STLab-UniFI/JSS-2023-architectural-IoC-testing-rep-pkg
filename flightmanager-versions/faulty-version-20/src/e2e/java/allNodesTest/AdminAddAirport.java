...
import it.unifi.ing.swam.controller.AirportController;
...
public class AdminAddAirport {
	...
	public static WebArchive createDeployment() {
		...
		File[] awaitility = Maven.resolver().loadPomFromFile("pom.xml").resolve("org.awaitility:awaitility:4.2.0")
				.withoutTransitivity().asFile();
		...
		WebArchive war = ShrinkWrap.create(WebArchive.class, "search.war").addPackages(true, "it.unifi.ing.swam")
				...
				.addAsLibraries(primefaces).addAsLibraries(awaitility);
	...
	public void test1() {
		...
		Warp.initiate(new Activity() {
			...
			@AfterServlet
			public void checkNewAiportId() {
				assertNull(airportController.getAirport().getId());
			}
		});
